...
public class ListaOdListi {
    ...
    public static long findMagicNumber(DLL<DLL<Integer>> list) {
		...
		DLLNode<Integer> currInt;
		DLL<Integer> products = new DLL<Integer>();
		while(currList != null){
			currInt = currList.element.getFirst();
			int sum = 0;
			while( currInt != null ){
				sum += currInt.element;
				currInt = currInt.succ;
			}
			products.insertLast(sum);
			currList = currList.succ;
		}
		long product = 1;
		currInt = products.getFirst();
		while(currInt != null){
			product *= currInt.element;
			currInt = currInt.succ;
		}
		return product;
	}
