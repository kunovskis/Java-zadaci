...
public class TancSoStudentite {
    private static Semaphore maleChange, femaleChange, danceFloor, ladyReady;

    public void init() {
        maleChange = new Semaphore(Show.BROJ_GARDEROBA);
        femaleChange = new Semaphore(Show.BROJ_GARDEROBA);
        danceFloor = new Semaphore(Show.BROJ_TEREN);
        ladyReady = new Semaphore(0);
    }
    ...
    class Zensko extends Thread {
        ...
        public void ucestvo() throws InterruptedException {
            femaleChange.acquire();
            show.presobleci();
            femaleChange.release();
            ladyReady.release();
        }
