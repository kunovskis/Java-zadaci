...
public class DLLVojska {
	...
	private static DLL<Integer> vojska(DLL<Integer> lista, int a, int b, int c, int d) {
        ...
        DLL<Integer> node = new DLL<Integer>();
        node.insertFirst(a);
        node.insertAfter(b, node.getFirst());
        node.insertAfter(c, node.getFirst().succ);
        node.insertAfter(d, node.getFirst().succ.succ);
        DLLNode<Integer> tmp = node.getFirst();
        for(int i=0; i<4; i++)
        {
